...
const Login = () => {
  ...
  useEffect(() => {
    if (location.state) {
      if (location.state.packageId) {
        ...
        console.log(
          "Pre-filled packageId from navigation state:",
      ...
      if (location.state.paymentConfirmed) {
        ...
        showCustomMessageBox(
          "Payment Required",
          "Please complete the payment for your selected package first."
        );
        navigate("/payment", {
  ...
  const showCustomMessageBox = (title, message) => {
    // In a real application, you would render a modal or a toast notification here.
    // For this example, we'll just log to console and use a simple alert fallback.
    console.log(`${title}: ${message}`);
    alert(`${title}\n\n${message}`); // Fallback to alert for immediate feedback
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      try {
        ...
        if (!response.ok) {
          showCustomMessageBox("Login Failed", data.error || "Login failed");
          throw new Error(data.error || "Login failed");
        }

        localStorage.setItem("token", data.token);
        // 🌟 UPDATED: Store the full user object received from the backend 🌟
        ...
        const response = await fetch(
          "http://localhost:5000/api/users/register",
          {
            ...
            body: JSON.stringify({
              ...
              cinicNumber: formData.cnic, // Corrected from 'cnic' to 'cinicNumber' as per schema
              email: formData.email,
              password: formData.password,
                ...
                {!paymentConfirmed && formData.packageId && (
                  ...
                  >
                    ...
                    <span className="block sm:inline">
                      {" "}
                      Please complete payment for the{" "}
                    ...
                    <button
                      onClick={() =>
                        navigate("/payment", {
                          state: { packageId: formData.packageId },
                        })
                      }
                      type="button"
